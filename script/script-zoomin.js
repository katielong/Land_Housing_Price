...
function chart1(d) {
    ...
    var line = d3.line()
        ...
        .y(function(d) {
            return yScale(d.housing_price); }),

        line2 = d3.line()
        .x(function(d) {
            return xScale(d.year) })
        .y(function(d) {
            return yScale2(d.resid_price); });
...
function chart2(d) {
    ...
    svg2.selectAll(".bar")
        ...
        .append("rect")
        .attr("x", function(d) {
            return xScale_bar(d.year); })
        .attr("width", "39px")
        .attr("y", function(d) {
            return yScale(d.resid_area); })
        .attr("height", function(d) {
            return height - yScale(d.resid_area); });
}
    ...
    function lines(d) {
        var id = d.id,
            target = data.filter(function(d) {
                return d.cityid === id; })[0];
        var out = [];
        var f2 = d3.format(".0f");
        for (i = 0; i <= 5; i++) {
            out[i] = {
                housing_price: +target.housing_price[i] / 100,
